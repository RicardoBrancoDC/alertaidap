...
def http_get(url: str, timeout: int = HTTP_TIMEOUT) -> bytes:
    """
    GET com retry simples, para lidar com instabilidade e IncompleteRead em chunked.
    """
    ...
    for attempt in range(1, HTTP_RETRIES + 1):
        try:
            ...
            last_err = e
            print(f"[http_get] IncompleteRead na tentativa {attempt}/{HTTP_RETRIES}: {e}. Vou tentar de novo.")
        except urllib.error.HTTPError as e:
            # se for erro do servidor, pode valer retry; se for 4xx, normalmente não
            body = ""
            try:
            ...
            print(f"[http_get] HTTPError na tentativa {attempt}/{HTTP_RETRIES}: {e.code}.")
            if 400 <= e.code < 500:
                # 4xx não costuma melhorar com retry
                break
        except urllib.error.URLError as e:
            last_err = e
            print(f"[http_get] URLError na tentativa {attempt}/{HTTP_RETRIES}: {e}. Vou tentar de novo.")
        except Exception as e:
            last_err = e
            print(f"[http_get] Erro inesperado na tentativa {attempt}/{HTTP_RETRIES}: {e}. Vou tentar de novo.")
...
def tg_send_message(text: str) -> None:
    if not TG_TOKEN or not TG_CHAT_ID:
        print("Telegram não configurado (TELEGRAM_BOT_TOKEN/TELEGRAM_CHAT_ID). Mensagem:")
        print(text)
        return
...
def chunk_text(text: str, limit: int = 3900) -> List[str]:
    """
    Telegram limite 4096, deixo margem.
    Corta também linhas gigantes (headline enorme etc).
    """
    parts: List[str] = []
    buf = ""
...
# =========================
# Business rules: nível do alerta
# =========================
def calc_nivel(severity: str, urgency: str, certainty: str, response_type: str) -> str:
    ...
    # Médio / Baixo: só severity
    if s == "Moderate":
        return "Médio"
    if s == "Minor":
        return "Baixo"

    # Severo e Alto: severity=Severe
    if s == "Severe":
...
def main() -> int:
    ...
    # marca como visto tudo que está no feed agora
    for e in entries:
